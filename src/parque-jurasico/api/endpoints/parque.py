...
async def obtener_diseno_parque(usuario_actual: models.UsuarioAuth = Depends(seguridad.obtener_usuario_actual)):
    return db.DISENO_DEL_PARQUE

@router_parque.get("/dinosaurio/{dino_id}", response_model=modelos.Dinosaurio)
async def obtener_dinosaurio(dino_id: str, usuario_actual: models.UsuarioAuth = Depends(seguridad.obtener_usuario_actual)):
